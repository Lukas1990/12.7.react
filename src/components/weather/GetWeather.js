...
function GetWeather(props) {
  ...
  const getData = () => {
    let option = select.current.selectedOptions[0]

    city.current.innerText = select.current.value

    let url = "https://api.open-meteo.com/v1/forecast?latitude="+option.dataset.latitude+"&longitude="+option.dataset.longitude+"&daily=weathercode,temperature_2m_max,rain_sum,snowfall_sum,windspeed_10m_max&timezone=Europe%2FBerlin&forecast_days=3&start_date="+today+"&end_date="+futureDate

    axios.get(url).then(response => {
      let data = []
      let day_data = response.data.daily

      for (let i = 0; i < day_data.time.length; i++) {
        let date = day_data.time[i].split("-")
        date = date[2]+". "+date[1]+". "+date[0]

        data.push({
          rain_sum: day_data.rain_sum[i],
          snowfall_sum: day_data.snowfall_sum[i],
          temperature_2m_max: day_data.temperature_2m_max[i],
          time: date,
          weathercode: day_data.weathercode[i],
          windspeed_10m_max: day_data.windspeed_10m_max[i]
        })
      }
      onSubmitData(data)
    })
  ...
  return (
    <div>
      <p>{phrase["Find out the weather"]} <strong ref={city}>Košice</strong>
      </p>
